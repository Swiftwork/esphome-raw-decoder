...
function findCommonSuffix(binaries) {
  ...
  for (let i = firstBinary.length - 1; i >= 0; i--) {
    const char = firstBinary[i];
    if (binaries.every(binary => binary[binary.length - 1 - (firstBinary.length - 1 - i)] === char)) {
      suffix = char + suffix;
    } else {
      break;
    }
...
function splitSignals(filteredSignals) {
  ...
  const maxStaticLength = Math.min(commonPrefix.length + commonSuffix.length, 
  ...
  if (commonPrefix.length + commonSuffix.length > maxStaticLength) {
    ...
    finalSuffix = commonSuffix.slice(Math.max(0, commonPrefix.length + commonSuffix.length - maxStaticLength));
  }
  
  const staticPart = finalPrefix + (finalSuffix && finalPrefix !== finalSuffix ? finalSuffix : "");
...
export async function decodeAction(file, options) {
  ...
  try {
    ...
    if (signalFrequency.size > 0) {
      ...
      const filteredSignals = Array.from(signalFrequency.entries())
        .filter(([binary, count]) => count >= minOccurrences);
        ...
        if (splitEnabled && filteredSignals.length > 1) {
          ...
          if (staticPart.combined.length > 0) {
            ...
            if (staticPart.prefix.length > 0) {
              console.log(`  Prefix:  ${chalk.white(staticPart.prefix)} (${staticPart.prefix.length} bits)`);
            }
            if (staticPart.suffix.length > 0) {
              console.log(`  Suffix:  ${chalk.white(staticPart.suffix)} (${staticPart.suffix.length} bits)`);
            }
            console.log(`  Combined: ${chalk.white(staticPart.combined)} (${staticPart.combined.length} bits)`);
            
            let staticHex;
            try {
              staticHex = staticPart.combined.length > 0 ? BigInt("0b" + staticPart.combined).toString(16).toUpperCase() : "N/A";
            } catch (error) {
              staticHex = "INVALID";
            }
          ...
          splitResults.forEach((result, index) => {
            ...
            let variableHex;
            try {
              variableHex = variablePart.length > 0 ? BigInt("0b" + variablePart).toString(16).toUpperCase() : "N/A";
            } catch (error) {
              variableHex = "INVALID";
            }
            const percentage = ((count / signals.length) * 100).toFixed(1);
            
            console.log(`${chalk.green((index + 1).toString().padStart(2, ' '))}. Variable: ${chalk.white(variablePart)} (0x${chalk.yellow(variableHex)}) | Full: ${chalk.gray(binary)} - ${chalk.blue(count)} occurrences (${chalk.magenta(percentage)}%)`);
          });
        } else {
          ...
          filteredSignals.forEach(([binary, count], index) => {
            let hex;
            try {
              hex = BigInt("0b" + binary).toString(16).toUpperCase();
            } catch (error) {
              hex = "INVALID";
            }
            const percentage = ((count / signals.length) * 100).toFixed(1);
            console.log(`${chalk.green((index + 1).toString().padStart(2, ' '))}. ${chalk.white(binary)} (0x${chalk.yellow(hex)}) - ${chalk.blue(count)} occurrences (${chalk.magenta(percentage)}%)`);
          });
      ...
      console.log(`Signals meeting threshold (≥${minOccurrences}): ${chalk.cyan(filteredSignals.length)}`);
    }
  } catch (error) {
