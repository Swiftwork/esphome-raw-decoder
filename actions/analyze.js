...
function displayHistogram(data, title, unit, bins = 10) {
  ...
  for (let i = 0; i < bins; i++) {
    ...
    console.log(`  ${rangeStart.toString().padStart(4)}-${rangeEnd.toString().padEnd(4)}${unit}: ${bar} (${count})`);
  }
  console.log();
}
...
async function parseRawSignalsForAnalysis(file, options) {
  return parseLogFile(file, (signal) => {
    ...
    if (numbers.length >= options.minLength && numbers.length <= options.maxLength) {
      ...
      let shortPulses = pulses.filter(p => p <= options.maxShortPulse && p >= options.minShortPulse);
      let longPulses = pulses.filter(p => p >= options.minLongPulse && p <= options.maxLongPulse);
...
export async function analyzeAction(file, options) {
  ...
  try {
    ...
    console.log(chalk.gray(`Pulse filtering: ${parseOptions.minShortPulse || 0}μs - ${parseOptions.maxShortPulse || '∞'}μs (short), ${
      parseOptions.minLongPulse || 0}μs - ${parseOptions.maxLongPulse || '∞'}μs (long)`));
    console.log(chalk.gray(`Signal length filtering: ${parseOptions.minLength} - ${parseOptions.maxLength} pulses`));
    ...
    console.log(chalk.green(`Found ${signals.length} valid signals to analyze\n`));
    ...
    if (allShortPulses.length > 0 && allLongPulses.length > 0) {
      ...
      console.log(`  Suggested short pulse threshold: ${Math.round(median(allShortPulses))}μs`);
      console.log(`  Suggested long pulse threshold: ${Math.round(median(allLongPulses))}μs`);
      
      // Calculate tolerance suggestion
      const shortVariability = standardDeviation(allShortPulses) / average(allShortPulses) * 100;
      const longVariability = standardDeviation(allLongPulses) / average(allLongPulses) * 100;
      const suggestedTolerance = Math.max(shortVariability, longVariability) * 1.5;
      
      console.log(`  Suggested --tolerance: ${Math.round(suggestedTolerance)}%`);
      console.log();
    }

    // Display histograms using the generic function
    if (options.histogram) {
      displayHistogram([...allShortPulses, ...allLongPulses], "PULSE DURATION HISTOGRAM", "μs");
